...
- 普通未跟踪文件仍会参与审查。按路径命中的 `.env`、私钥与密钥库、SSH 私钥、`.npmrc`、`.netrc`、`.aws/`、`.kube/`、Docker 凭证、Google Cloud 凭证、Terraform state/plan 和 `.zadig-review-agent/` 配置则始终跳过；规则文件的 `include` 不能覆盖这一限制。
- 仅当已跟踪的敏感文件相对 `HEAD` 被删除时，工作区才额外跳过**所有**未跟踪文件；同一变更中的新增文件也会跳过。可能的改名目标也会被只读工具拦截，并在报告中标为 `sensitive_rename_precaution`；审查以不完整状态退出，因为可能漏审无关的新文件。路径规则无法识别普通文件名中的密钥，这类文件仍需自行排除。
