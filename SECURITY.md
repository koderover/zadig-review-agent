...
Until the first tagged release, security fixes are applied to `main`. After releases begin, only the latest release line and `main` are supported unless a release note states otherwise.

在首个标签版本发布前，安全修复会应用到 `main`。开始发布后，除非发布说明另有声明，仅支持最新版本线和 `main`。
...
2. If private vulnerability reporting is unavailable, email `contact@koderover.com` with the subject `[zadig-review-agent security]`.
...
2. 如果私有漏洞报告不可用，请发送邮件至 `contact@koderover.com`，标题以 `[zadig-review-agent security]` 开头。
