...
- **Key checks**:
  - Suggest expression-body functions only when they materially improve changed code; do not report this as a correctness issue.
  - Replace complex `if-else` chains with `when`.
  - Report unnecessary labeled or non-local returns only when they obscure or change control flow.
- **Bad example**:
  ```kotlin
