...
#### Reliability
- **Missing timeout**: Flag missing `timeout-minutes` when a changed job can block, retry, deploy, or consume self-hosted runner capacity indefinitely
...
- **Incorrect cache reuse**: Cache keys or restore prefixes can reuse incompatible or untrusted artifacts across branches, platforms, or dependency versions
