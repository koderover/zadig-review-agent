...
#### Code Quality Checks
- **Duplicate Code**: Report duplication only when the current change creates a concrete maintenance or consistency risk
...
- **Variable Declarations**: Report `var` when function scoping or hoisting can change behavior; otherwise treat `let` or `const` as a non-blocking style preference
